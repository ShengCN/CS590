...
void solve(const std::vector<Vect3d> &points, int n, std::vector<hermite_curve> &curves) {
	...
	// initialize
	// todo, find knots
	curves[0] = hermite_curve(points.front(), points.back(), points.front(), points.back());

	MatrixXd phi_km(4, sample_num), m(sample_num,4);
	MatrixXd y(sample_num, 3);
	for (size_t si = 0; si < sample_num; ++si) {
		float t; Vect3d tmp;
		curves[0].find_closet_point(points[si], t, tmp);
	...
	curves[0].coefficients = Matrix4d(p0, p1, p2, p3);
}